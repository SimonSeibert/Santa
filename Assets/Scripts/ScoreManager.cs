...
public class ScoreManager : MonoBehaviour
{
    ...
    void Start()
    {
        StartCoroutine("LateStartTextInit", .1f);
    }

    //Execute that a bit later to make sure that the Data is set
    IEnumerator LateStartTextInit(float waitTime)
    {
    ...
    {
        //Only score when the game is active
        if (Data.Instance.getIsGameActive())
        {
            //Success
            if (success)
            {
